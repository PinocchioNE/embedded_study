...
#define W25Q64_EN
		...
		W25Q64_ReadID(&MID,&DID);//EF 4017
		OLED_ShowString(0, 0, "MID:   DID:", OLED_6X8);//6x8字体 纵向占8个点 横向6个点 屏幕128x64  （x,y）
		OLED_ShowHexNum(24, 0, MID, 2, OLED_6X8);//4*6=24 一个字符横向6个点
		OLED_ShowHexNum(66, 0, DID, 4, OLED_6X8);//11*6=66
		
		
		#define W25Q64_WRITE_EN
		#ifdef W25Q64_WRITE_EN
			uint8_t ArrayWrite[] = {0x01, 0x02, 0x03, 0x04};
			uint8_t ArrayRead[4];
			OLED_ShowString(0, 8, "W:", OLED_6X8);
			OLED_ShowString(0, 16, "R:", OLED_6X8);
			...
			for(uint8_t i = 0; i < 4; i++)
			{
				OLED_ShowHexNum((12+i*18), 8, ArrayWrite[i], 2, OLED_6X8);//(12, 8)

			}
			
			for(uint8_t i = 0; i < 4; i++)
			{
				OLED_ShowHexNum((12+i*18), 16, ArrayRead[i], 2, OLED_6X8);
			}			
...
/******************************************************************************************************************************************/	
	...
	#ifdef NM25Q128_EN
		NM25Q128_Init();	
		NM25Q128_ReadID(&MID, &DID, NM25Q128_CMD_READ_JEDEC_ID);// NM25Q128_CMD_READ_JEDEC_ID  NM25Q128_CMD_READ_MANUFACTURER_ID
		OLED_ShowHexNum(1, 1, MID, 2, OLED_6X8);
		OLED_ShowHexNum(20, 1, DID, 4, OLED_6X8);
	#endif
